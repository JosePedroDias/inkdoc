...
var generateMarkup = function(root, cfg, cb) {
    ...
    var writeTidy = function(fileName, content, cb) {
        ...
        fs.writeFile(path.resolve(process.cwd(), fileName), content, cb);
    };
    ...
    hb.registerHelper('stripP', function(text, options) {
        return text.replace('<p>','').replace('</p>', '').trim().replace(/(\r\n|\n|\r)/gm, '<br>');
    });

    hb.registerHelper('countLevel', function(text) {
        text = text || '';
        return text.split('.').length;
    });

    hb.registerHelper('firstP', function(text, options) {
        return text.replace('<p>','').replace('</p>', '').split('\n')[0];
    });
